...
import cn.gjp0609.ams.utils.SecurityUtils;
...
//        String vcode = SecurityUtils.getRandomCode(4);
        ServletActionContext.getRequest().getSession().removeAttribute(name + "VCode");
        // 将有标识的随机码存入 session 作用域
        setSessionValue(name + "VCode", code);
        setApplicationValue("vc", code);
        System.out.println("          vcode -> " + name + " -> " + code);
        System.out.println("     " + getSessionValue(name + "VCode"));
        // 用获取的随机码生成验证码图片
